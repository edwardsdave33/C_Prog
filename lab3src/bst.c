...
{ 
	...
	return(intmax(leftheight,rightheight)); 
}//height()
...
// at least one child (see findParent()).
{
	...
	if (root->rightChild->key == k || root->leftChild->key == k){
		return root;
	}
	else{
		if (k > root->key){
...
//	p	- points to parent of node to be deleted.
{
   ...
   if(ptr->key > p->key){
	   p->rightChild=ptr;
	   ptr->leftChild=n->leftChild;
	   ptr->rightChild=n->rightChild;
	   free(n);
   }else{
	   p->leftChild=ptr;
	   ptr->leftChild=n->leftChild;
