...
// completes the index calculation.)
{
	...
	sum+= (long)(x[i] * (pow(31,strlen(x)-1-i)));
	// ---<SNIP>---
	}
	return sum;
}//h()
...
//insert into the linked list using add front method
}
...
int findInTable(HashTable *ht, char* x)
{
	//#########################COMPLETE THIS FUNCTION##########################
	// ---<SNIP>---
// find hash code of string, index to that linked list and search if is there
int index= hash(x, ht->size);
return find(ht->a[index],x);
