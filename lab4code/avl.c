...
/*
if(calcHeight(x->rightChild) > calcHeight(x->leftChild)){
	printf("Node %d is unbalanced. Right of Left: Rotate left Rotate Right", root->key);
	Node* LL = rotateLeft(x);
	Node *NewL= LL->rightChild;
	LL->rightChild = root;
	root->leftChild= NewL;
	root=LL;
	*/
	//enter code for regular right roation


//	printf("Node %d is unbalanced. Left of left: Rotate Right \n", root->key);
Node *NewL= x->rightChild;
x->rightChild = root;
root->leftChild=NewL;
...
/*
if(calcHeight(x->rightChild) < calcHeight(x->leftChild)){
	printf("Node %d is unbalanced. Left of Right: Rotate Right Rotate left", root->key);
	Node* RR = rotateRight(x);
	Node *NewR= RR->leftChild;
	RR->leftChild = root;
	root->rightChild=NewR;
	root=RR;
	*/
Node* x = root->rightChild;
	//printf("Node %d is unbalanced. Right of Right: Rotate Left \n", root->key);
	Node* NewR= x->leftChild;
	x->leftChild = root;
	root->rightChild=NewR;
	x->height=calcHeight(x);
	root->height=calcHeight(root);
...
if(bf > 1){
	...
	int bf2= getBalanceFactor(root->leftChild);
	if(bf2<=-1){
		printf("Node %d is unbalanced. Right of Left: Rotate Left, Rotate right \n", root->key);
	...
	int bf2= getBalanceFactor(root->rightChild);
	if(bf2>=1){
		printf("Node %d is unbalanced. Left of Right: Rotate Right, Rotate Left \n", root->key);
